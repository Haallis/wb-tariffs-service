...
export async function getSheetsClient() {
    try {
        const auth = new google.auth.GoogleAuth({
            keyFile: process.env.GOOGLE_SERVICE_ACCOUNT_FILE_PATH,
            scopes: ["https://www.googleapis.com/auth/spreadsheets"],
        });
        const sheets = google.sheets({ version: "v4", auth });
        return sheets;
    } catch (error) {
        console.error("Ошибка подключения к API google sheets", error);
    }
}
...
export async function updateTariffsInSheets() {
    try {
        ...
        const defaultSheetId = "1a25yUuimVxgyMyFnOhcw6EJMJ-VogiVqzFS6jHZ2Hsk";
        const sheetIds = process.env.GOOGLE_SHEET_IDS || defaultSheetId;
        ...
        console.log("Все gooogle таблицы успешно обновлены");
    } catch (error) {
        console.error(error);
    }
}
